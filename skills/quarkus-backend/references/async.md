...
Use an outbox table when the domain write and event publication must stay consistent:

```java
@Transactional
public {Entity}Dto create(Create{Entity}Request request) {
    var saved = doCreate(request);
    outboxRepository.save(OutboxMessage.of("{entity}.created.v1", saved.id().value(), saved));
    return mapper.toDto(saved);
}
```

The Kafka publisher then reads pending outbox rows and publishes them asynchronously.

### DLQ / Dead Letter Handling

When a consumer cannot process a message after bounded retries:

- preserve the original payload and key
- add failure metadata (`reason`, `service`, `attempts`)
- publish to a dedicated dead-letter topic
- never drop the message silently

### Partition Keys

- Use a stable business identifier as the Kafka key (`{entity}Id`, contract id, shipper id).
- Do not use random UUID keys for ordered aggregates.
- All events for the same aggregate must land on the same partition.

### Retry and Backoff

- Retry only transient failures (network, timeout, temporary downstream unavailability).
- Do not retry validation or schema errors.
- Use bounded retries with backoff, then route to DLQ.

### Schema Evolution

- Version topic payloads explicitly (`.v1`, `.v2`).
- Prefer additive schema changes.
- Do not repurpose existing fields with new semantics.
- Document compatibility expectations before changing serializers.

### Idempotency Keys

- Use the event id or aggregate version as the idempotency key.
- Persist processed event ids when consumer side effects are non-repeatable.
- Treat "already processed" as a success path, not as an error.
...
- **Kafka producers** that must stay consistent with DB writes should prefer the outbox pattern over direct in-transaction publication.
- **Dead-letter routing** must preserve payload, key, and failure metadata for replayability.
- **Partition keys** must align with aggregate ordering requirements.
- **Schema changes** must be versioned and backward-compatible by default.
